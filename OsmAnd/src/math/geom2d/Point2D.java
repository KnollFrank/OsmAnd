...
public class Point2D implements GeometricObject2D, PointShape2D, Cloneable,
        CirculinearShape2D, Serializable {
    ...
    public static Point2D fromPointF(final PointF p) {
        return new Point2D(p.x, p.y);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public static Point2D fromSizeF(final SizeF size) {
        return new Point2D(size.getWidth(), size.getHeight());
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public static Point2D fromSize(final Size size) {
        return new Point2D(size.getWidth(), size.getHeight());
    }
