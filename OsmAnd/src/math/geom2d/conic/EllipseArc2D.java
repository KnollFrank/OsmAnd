 ...
 * @author dlegland
 */
public class EllipseArc2D extends AbstractSmoothCurve2D implements
        SmoothOrientedCurve2D, EllipseArcShape2D, Cloneable {
    ...
    public boolean containsAngle(double angle) {
        return Angle2D.containsAngle(startAngle, startAngle + angleExtent,
                angle, angleExtent > 0);
    }
    ...
    public CurveSet2D<? extends EllipseArc2D> clip(Box2D box) {
        ...
        CurveArray2D<EllipseArc2D> result = new CurveArray2D<EllipseArc2D>(
                set.size());
    ...
    public Box2D boundingBox() {
        ...
        if (Angle2D.containsAngle(startAngle, endAngle, PI / 2 + ellipse.theta,
                direct))
    ...
    public Path appendPath(Path path) {
        ...
        for (int i = 0; i < nSeg; i++) {
            ...
            path.rCubicTo((float) p1.x() + (float) v1.x(), (float) p1.y()
                    + (float) v1.y(), (float) p2.x() - (float) v2.x(),
                    (float) p2.y() - (float) v2.y(), (float) p2.x(),
                    (float) p2.y());
        }
        return path;
    }
    ...
    public void draw(Canvas g2, Paint p) {
        g2.drawPath(this.getGeneralPath(), p);
    }
    ...
    public boolean almostEquals(GeometricObject2D obj, double eps) {
        ...
        if (!Angle2D.equals(startAngle, arc.startAngle))
            return false;
        return Angle2D.equals(angleExtent, arc.angleExtent);
    }
    ...
    public String toString() {
        ...
        return String.format(Locale.US,
                ...
                center.x(), center.y(), ellipse.r1, ellipse.r2, ellipse.theta,
    ...
    public boolean equals(Object obj) {
        ...
        if (!EqualUtils.areEqual(startAngle, that.startAngle))
            return false;
        return EqualUtils.areEqual(angleExtent, that.angleExtent);
    }
