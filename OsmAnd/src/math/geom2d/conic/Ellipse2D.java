 ...
 */
public class Ellipse2D extends AbstractSmoothCurve2D implements EllipseShape2D,
        Cloneable {
    ...
    public static Ellipse2D reduceCentered(double[] coefs) {
        ...
        // assert abs(coefs2[1] / f) < Shape2D.ACCURACY : "Second conic coefficient should be zero";
        ...
        for (int iterations = 0; iterations < 100; ++iterations) {
            ...
            if (D >= 0) {
                ...
                tildeT = ((rPr > 0) ? pow(rPr, ot) : -pow(-rPr, ot))
                        + ((rMr > 0) ? pow(rMr, ot) : -pow(-rMr, ot)) - b * ot;
                ...
                if ((tildePhi * phi) < 0) {
                    ...
                    tildePhi = atan2(z * tildeT2P1 - 2 * k * tildeT, g2
                            * (r * tildeT2P1 - k * (1.0 - tildeT2)));
                    if (tildePhi * phi < 0) {
                        tildeT = 2 * qRoot * cos((alpha + 4 * PI) * ot) - b
                                * ot;
                        tildeT2 = tildeT * tildeT;
                        tildeT2P1 = 1.0 + tildeT2;
                        tildePhi = atan2(z * tildeT2P1 - 2 * k * tildeT, g2
    ...
    public Vector2D tangent(double t) {
        ...
        if (direct)
            return new Vector2D(-r1 * sin(t) * cot - r2 * cos(t) * sit, -r1
                    * sin(t) * sit + r2 * cos(t) * cot);
        else
            return new Vector2D(r1 * sin(t) * cot + r2 * cos(t) * sit, r1
                    * sin(t) * sit - r2 * cos(t) * cot);
    }
    ...
    public Point2D point(double t) {
        ...
        double cot = cos(theta);
        double sit = sin(theta);
        return new Point2D(xc + r1 * cos(t) * cot - r2 * sin(t) * sit, yc + r1
                * cos(t) * sit + r2 * sin(t) * cot);
    }
    ...
    public CurveSet2D<? extends SmoothOrientedCurve2D> clip(Box2D box) {
        ...
        CurveArray2D<SmoothOrientedCurve2D> result = new CurveArray2D<SmoothOrientedCurve2D>(
                set.size());
    ...
    public void draw(Canvas g2, Paint p) {
        g2.rotate((float) theta, (float) (xc - r1), (float) (yc - r2));
        g2.drawOval(new RectF((float) (xc - r1), (float) (yc - r2),
                (float) (xc + r1), (float) (yc + r2)), p);
    ...
    public String toString() {
        return String.format("Ellipse2D(%f,%f,%f,%f,%f,%s)", xc, yc, r1, r2,
                theta, direct ? "true" : "false");
    }
