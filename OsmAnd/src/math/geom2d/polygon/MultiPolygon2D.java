...
public class MultiPolygon2D implements Domain2D, Polygon2D {
    ...
    public void addVertex(Point2D position) {
        ...
        if (rings.size() == 0) {
            throw new RuntimeException(
                    "Can not add a vertex to a multipolygon with no ring");
        }
    ...
    public void insertVertex(int index, Point2D point) {
        ...
        if (nv <= index) {
            throw new IllegalArgumentException(
                    "Can not insert vertex at position " + index + " (max is "
                            + nv + ")");
        }
    ...
    public CirculinearContourArray2D<LinearRing2D> boundary() {
        return new CirculinearContourArray2D(rings);
    }
    ...
    public Polygon2D complement() {
        ...
        ArrayList<LinearRing2D> reverseLines = new ArrayList<LinearRing2D>(
                rings.size());
    ...
    public MultiPolygon2D transform(AffineTransform2D trans) {
        ...
        ArrayList<LinearRing2D> transformed = new ArrayList<LinearRing2D>(
                rings.size());
    ...
    public void draw(Canvas g2, Paint p) {
        g2.drawPath(this.boundary().getGeneralPath(), p);
    }
    ...
    public MultiPolygon2D clone() {
        ...
        ArrayList<LinearRing2D> array = new ArrayList<LinearRing2D>(
                rings.size());
