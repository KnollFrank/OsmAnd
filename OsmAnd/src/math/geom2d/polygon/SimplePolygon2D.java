...
public class SimplePolygon2D implements Polygon2D {
    ...
    public SimplePolygon2D simplify(double distMax) {
        return new SimplePolygon2D(Polylines2D.simplifyClosedPolyline(
                this.vertices, distMax));
    }
    ...
    public CirculinearDomain2D transform(CircleInversion2D inv) {
        CirculinearBoundary2D boundary = this.boundary().transform(inv)
                .reverse();
    ...
    public CirculinearDomain2D buffer(double dist) {
        ...
        if (PointSets2D.hasMultipleVertices(this.vertices, true)) {
            List<Point2D> pts2 = PointSets2D.filterMultipleVertices(
                    this.vertices, true);
    ...
    public CirculinearContourArray2D<LinearRing2D> boundary() {
        ...
        return new CirculinearContourArray2D<LinearRing2D>(new LinearRing2D(
                array));
    }
