...
public final class Polygons2D {
    ...
    public final static SimplePolygon2D createRectangle(double x1, double y1,
        ...
        return new SimplePolygon2D(new Point2D(xmin, ymin), new Point2D(xmax,
                ymin), new Point2D(xmax, ymax), new Point2D(xmin, ymax));
    }
    ...
    public final static SimplePolygon2D createCenteredRectangle(Point2D center,
        ...
        return new SimplePolygon2D(
                new Point2D(x1, y1), new Point2D(x2, y1),
    ...
    public final static SimplePolygon2D createOrientedRectangle(Point2D center,
        ...
        return new SimplePolygon2D(new Point2D(-len * cot + wid * sit + xc, -len * sit - wid * cot
                + yc),
                new Point2D(len * cot + wid * sit + xc, len * sit - wid * cot
                        + yc),
                new Point2D(len * cot - wid * sit + xc, len * sit + wid * cot
                        + yc),
                new Point2D(-len * cot - wid * sit + xc, -len * sit + wid * cot
    ...
    private final static int isLeft(Point2D p1, Point2D p2, Point2D pt) {
        double x = p1.x();
        double y = p1.y();
        return (int) Math.signum((p2.x() - x) * (pt.y() - y) - (pt.x() - x)
                * (p2.y() - y));
    }
     ...
     */
    public final static Polygon2D convexHull(
    ...
    public final static Polygon2D clipPolygon(Polygon2D polygon, Box2D box) {
        ...
        ContourArray2D<Contour2D> contours = Boundaries2D.clipBoundary(
                boundary, box);
