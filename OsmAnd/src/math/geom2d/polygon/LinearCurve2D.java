 ...
 * 
 */
public abstract class LinearCurve2D extends AbstractContinuousCurve2D implements
     ...
     * 
     * @param vertex
     *            the position of the vertex to remove
    ...
    /**
     * Returns a collection of LineSegment2D that represent the individual edges
    ...
    public CirculinearDomain2D buffer(double dist) {
        ...
        if (PointSets2D.hasMultipleVertices(this.vertices)) {
            Polyline2D poly2 = Polyline2D.create(PointSets2D
                    .filterMultipleVertices(this.vertices));
    ...
    public CurveSet2D<? extends LinearCurve2D> clip(Box2D box) {
        ...
        CurveArray2D<LinearCurve2D> result = new CurveArray2D<LinearCurve2D>(
                set.size());
    ...
    public void draw(Canvas g2, Paint p) {
        g2.drawPath(this.asGeneralPath(), p);
    }
