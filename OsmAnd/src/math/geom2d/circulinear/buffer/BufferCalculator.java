...
public class BufferCalculator {
    ...
    public CirculinearCurve2D createParallel(CirculinearCurve2D curve,
        ...
        for (CirculinearContinuousCurve2D continuous : curve.continuousCurves()) {
            CirculinearContinuousCurve2D contParallel = createContinuousParallel(
                    continuous, dist);
    ...
    public CirculinearBoundary2D createParallelBoundary(
        ...
        Collection<? extends CirculinearContour2D> contours = boundary
                .continuousCurves();

        // allocate the array of parallel contours
        Collection<CirculinearContour2D> parallelContours = new ArrayList<CirculinearContour2D>(
                contours.size());
        ...
        return new CirculinearContourArray2D(parallelContours);
    }
    ...
    public CirculinearContinuousCurve2D createContinuousParallel(
        ...
        // curves
        return new PolyCirculinearCurve2D(parallelCurves, curve.isClosed());
    }
    ...
    private Collection<CirculinearContinuousCurve2D> getParallelElements(
        ...
        Collection<? extends CirculinearElement2D> elements = curve
                .smoothPieces();
    ...
    public CirculinearDomain2D computeBuffer(CirculinearCurve2D curve,
        ...
        for (CirculinearContinuousCurve2D cont : curve.continuousCurves()) {
            ...
            for (CirculinearContinuousCurve2D splitted : CirculinearCurves2D
        ...
        for (CirculinearContour2D contour : contours) {
            ...
            // remove intersection points that are vertices of the reference
            // curve
     ...
     */
    private Collection<? extends CirculinearContour2D> computeBufferSimpleCurve(
        ...
        if (curve.isClosed()) {
            ...
            contours.addAll(createSingleContourFromTwoParallels(parallel1,
                    parallel2));
        }

        // some contours may intersect, so we split them
        Collection<CirculinearContour2D> contours2 = removeIntersectingContours(
                contours, curve, d);
     ...
     */
    private Collection<CirculinearContour2D> createSingleContourFromTwoParallels(
    ...
    private Collection<CirculinearContour2D> removeIntersectingContours(
        ...
        for (CirculinearContour2D contour : contours)
            ...
            for (CirculinearContinuousCurve2D splitted : CirculinearCurves2D
                ...
                double dist = CirculinearCurves2D.getDistanceCurvePoints(curve,
                        splitted.singularPoints());
    ...
    private CirculinearContour2D convertCurveToBoundary(
        ...
        // if the curve is closed, return an instance of
        // GenericCirculinearRing2D
        if (curve.isClosed())
            return new GenericCirculinearRing2D(curve.smoothPieces());

        return new BoundaryPolyCirculinearCurve2D(curve.smoothPieces());
    }

    private double getDistanceCurveSingularPoints(CirculinearCurve2D ref,
