 ...
 */
public class StraightLine2D extends AbstractLine2D implements SmoothContour2D,
    ...
    public static StraightLine2D createParallel(LinearShape2D linear, double d) {
        ...
        return new StraightLine2D(line.x0 + line.dy * d2, line.y0 - line.dx
                * d2, line.dx, line.dy);
    }
    ...
    public StraightLine2D transform(AffineTransform2D trans) {
        ...
        return new StraightLine2D(x0 * tab[0] + y0 * tab[1] + tab[2], x0
                * tab[3] + y0 * tab[4] + tab[5], dx * tab[0] + dy * tab[1], dx
                * tab[3] + dy * tab[4]);
    }
    ...
    public boolean almostEquals(GeometricObject2D obj, double eps) {
        ...
        if (Math.abs(dx - line.dx) > eps)
            return false;
        return !(Math.abs(dy - line.dy) > eps);
    }
    ...
    public String toString() {
        return "StraightLine2D(" + x0 + "," + y0 + "," + dx + ","
                + dy + ")";
    }
