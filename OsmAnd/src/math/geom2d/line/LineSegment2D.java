...
public class LineSegment2D extends AbstractLine2D implements Cloneable,
    ...
    public static StraightLine2D getMedian(LineSegment2D edge) {
        return new StraightLine2D(edge.x0 + edge.dx * .5, edge.y0 + edge.dy
                * .5, -edge.dy, edge.dx);
    }
    ...
    public LineSegment2D parallel(double d) {
        ...
        d2 = d / d2;
        return new LineSegment2D(x0 + dy * d2, y0 - dx * d2, x0 + dx + dy * d2,
                y0 + dy - dx * d2);
    }
     ...
     * 
     * @see
     * math.geom2d.domain.OrientedCurve2D#signedDistance(math.geom2d.Point2D)
     */
    @Override
    ...
    public Path appendPath(Path path) {
        path.lineTo((float) x0 + (float) dx, (float) y0 + (float) dy);
        return path;
    }
    ...
    public boolean almostEquals(GeometricObject2D obj, double eps) {
        ...
        if (Math.abs(dx - edge.dx) > eps)
            return false;
        return !(Math.abs(dy - edge.dy) > eps);
    }
