 ...
 * is defined like other linear shapes: origin point, and direction vector.
 ...
 * @author dlegland
 */
public class LineArc2D extends AbstractLine2D implements SmoothOrientedCurve2D,
        Cloneable {
    ...
    public LineArc2D(Point2D point1, Point2D point2, double t0, double t1) {
        this(point1.x(), point1.y(), point2.x() - point1.x(), point2.y()
                - point1.y(), t0, t1);
    }
    ...
    /**
     * Returns the parameter of the first point of the line arc, arbitrarily set
     * to 0.
     */
    public double t0() {
        return t0;
    ...
    /**
     * Returns the parameter of the last point of the line arc, arbitrarily set
     * to 1.
     */
    public double t1() {
        return t1;
    ...
    public boolean isSingular(double pos) {
        if (Math.abs(pos - t0) < Shape2D.ACCURACY)
            return true;
        return Math.abs(pos - t1) < Shape2D.ACCURACY;
    }
    ...
    public LineArc2D transform(AffineTransform2D trans) {
        ...
        return new LineArc2D(x1, y1, dx * tab[0] + dy * tab[1], dx * tab[3]
                + dy * tab[4], t0, t1);
    }

    @Override
    public String toString() {
        return "LineArc2D(" + x0 + "," + y0 + "," + dx + "," + dy
                + "," + t0 + "," + t1 + ")";
    }
    ...
    public boolean almostEquals(GeometricObject2D obj, double eps) {
        ...
        if (!almostEquals(this.t0, that.t0, eps))
            return false;
        return almostEquals(this.t1, that.t1, eps);
    }

    /**
     * Compares two double values with a given accuracy, with correct result for
     * infinite values. Undefined results for NaNs.
     */
