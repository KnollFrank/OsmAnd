...
implements SmoothOrientedCurve2D, LinearElement2D {
	...
	public boolean isColinear(LinearShape2D linear) {
		...
		if (Math.abs(dx) > Math.abs(dy)) {
			return !(Math.abs((line.x0 - x0) * dy / dx + y0 - line.y0) > Shape2D.ACCURACY);
		} else {
			return !(Math.abs((line.y0 - y0) * dx / dy + x0 - line.x0) > Shape2D.ACCURACY);
		}
    }
	 ...
	 * belongs to the line, its location is given by x=x0+t*dx and y=y0+t*dy.
	 * <p>
