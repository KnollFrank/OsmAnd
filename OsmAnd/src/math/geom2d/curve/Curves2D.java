...
public abstract class Curves2D {
    ...
    public static CurveSet2D<ContinuousCurve2D> clipContinuousCurve(
        ...
        for (Point2D p : points)
            set.add(Double.valueOf(curve.position(p)));
    ...
    public static ContinuousCurve2D getFirstContinuousCurve(Curve2D curve) {
        ...
        Collection<? extends ContinuousCurve2D> curves = curve
                .continuousCurves();
        if (curves.size() == 0)
            return null;
    ...
    public static JunctionType getJunctionType(Curve2D prev, Curve2D next) {
        ...
        if (sp == 1 && sn == -1) {
            return abs(kappaPrev) < abs(kappaNext) ? JunctionType.SALIENT
                    : JunctionType.REENTRANT;
        } else if (sp == -1 && sn == 1) {
            return abs(kappaPrev) > abs(kappaNext) ? JunctionType.SALIENT
                    : JunctionType.REENTRANT;
        }
