...
/**
 * An array of domains. Note that this class if different from a generic domain
 * whose boundary is a set of contours. In the latter case, the shape is itself
 * a domain, not in the former.
 * 
 * @author dlegland
 * 
 */
public class DomainArray2D<T extends Domain2D> extends ShapeArray2D<T>
    ...
    public Boundary2D boundary() {
        ...
        ArrayList<Contour2D> boundaries = new ArrayList<Contour2D>(n);
            ...
            for (Contour2D contour : domain.boundary().continuousCurves()) {
                ...
                if (!contour.isClosed())
                    throw new IllegalArgumentException(
                            "Can not transform open curve to linear ring");
    ...
    public DomainArray2D<? extends Domain2D> transform(AffineTransform2D trans) {
        ...
        DomainArray2D<Domain2D> result = new DomainArray2D<Domain2D>(
                shapes.size());
