 ...
 * part of the boundary of a domain. Several OrientedCurve2D form a Contour2D,
 * and one or several Contour2D form a Boundary2D.
 * 
 * @author dlegland
 */
public interface OrientedCurve2D extends Curve2D {
    ...
    /**
     * Returns the signed distance of the curve to the given point. The distance
     * is positive if the point lies outside the shape, and negative if the
     * point lies inside the shape. In both cases, absolute value of distance is
    ...
    /**
     * Transforms the oriented curve, and returns another oriented curve. If
     * transform is not direct, the domains bounded by the transformed curve
     * should be complemented to have same orientation as the original domain.
     */
