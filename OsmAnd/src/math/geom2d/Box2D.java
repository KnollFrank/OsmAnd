...
public class Box2D implements GeometricObject2D, Cloneable {
    ...
    public SizeF getSize() {
        return new SizeF((float) getWidth(), (float) getHeight());
    }
    ...
    public Boundary2D boundary() {
        ...
        if (!bx0 && !bx1) {
            ...
            return new ContourArray2D<StraightLine2D>(new StraightLine2D(0, ymin, 1, 0),
        ...
        if (!by0 && !by1) {
            ...
            return new ContourArray2D<StraightLine2D>(new StraightLine2D(xmin, 0, 0, -1),
        ...
        if (bx0 && by0) // lower left corner
            return new BoundaryPolyCurve2D<LineArc2D>(new LineArc2D(xmin, ymin, 0, -1, NEGATIVE_INFINITY, 0),
                    new LineArc2D(xmin, ymin, 1, 0, 0, POSITIVE_INFINITY));

        if (bx1 && by0) // lower right corner
            return new BoundaryPolyCurve2D<LineArc2D>(new LineArc2D(xmax, ymin, 1, 0, NEGATIVE_INFINITY, 0),
                    new LineArc2D(xmax, ymin, 0, 1, 0, POSITIVE_INFINITY));

        if (bx1 && by1) // upper right corner
            return new BoundaryPolyCurve2D<LineArc2D>(new LineArc2D(xmax, ymax, 0, 1, NEGATIVE_INFINITY, 0),
                    new LineArc2D(xmax, ymax, -1, 0, 0, POSITIVE_INFINITY));

        if (bx0 && by1) // upper left corner
            return new BoundaryPolyCurve2D<LineArc2D>(new LineArc2D(xmin, ymax, -1, 0, NEGATIVE_INFINITY, 0),
